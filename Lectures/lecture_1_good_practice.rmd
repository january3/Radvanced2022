...
knitr::opts_chunk$set(echo=TRUE,warning=FALSE,message=FALSE,fig.width=5,fig.height=5,cache=FALSE,autodep=TRUE, results="hide",eval=FALSE)
```
...
https://github.com/january3/Radvanced2022

```{r,eval=TRUE,echo=FALSE}
library(qrcode)
plot(qr_code( "https://github.com/january3/Radvanced2022"))
```
 ...
 * first document the why, then the what and then the how
 * make documentation easily searchable (to use with grep, find & co)

## Rule #3: Stick to conventions

 * naming conventions: `PascalCase`, `camelCase`, `snake_case`
 * avoid dots in the middle of a variable name, they have special meaning
...
(Actually, nowadays it would not clash: you can have multiple objects with the same
name... if they are in different namespaces – but that is another story for
another time)
...
Note one interesting thing: this is an assignment operation, not a
declaration like in some other languages. `a` is a variable just like any
other, but instead of holding a number or a matrix it holds a function.
...
```{r}
stupid_add_1 <- function(a, b=3) {
  a + b
}

stupid_add_2 <- function(a, b=a) {
  a + b
}
...
Extracting arguments from ellipsis:

```{r}
ellipsis_check <- function(...) {
  arguments <- list(...)
...
Pass on additional arguments for wrapper functions:

```{r}
density_example <- function(family="normal", n=1000, ...) {
  family <- match.arg(family, c("normal", "uniform", "exponential"))
  func <- switch(family,
                 "normal"=rnorm,
                 "uniform"=runif,
                 "exponential"=rexp)
...
Whoever uses your function will be a malicious idiot indending to mess up
everything. Even if it just you.

 * Check arguments
 * If your function throws an error, stop your work and make sure that next time it
   would first check the arguments and report if anything is problematic
...
"<-"(no_way_this_works, 222)
```

## There is a difference between functions and variables
...
## but a is a variable *holding* a function!
...
## So, how about...
(function(x) { print(paste("Hello", x)) })("Bill")
```

## This is important!

 * We store functions in variables just like any other data type
 * how about a vector of functions?
 * how about passing a function as an argument to another function?

## Passing functions as arguments

```{r}
boring_data <- list(a=10, b=20, c=1:50)

Reduce(c, boring_data)
```
...
Which one will throw an error? Why? Why not?
 ...
 * There are no "scalars" in R (i.e., variables holding just one value)
 * What looks like a single value is a vector of length 1
