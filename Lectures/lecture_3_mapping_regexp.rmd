 ...
 * tidyverse notation with `~` is a bit more concise, but as any shorthand,
   it can be confusing (but you don't have to use it)
...
frame, vector etc.). Which sometimes fails. Avoid it.
...
```{r eval=FALSE}
## Replace "none" by NA 
starwars %>% mutate(across(everything(), ~ gsub("none", NA, .)))

## Modify all variable names
starwars %>% rename_with(~ paste0("Var_", .))
```

## Speaking of `rename_with`

`rename_with` applies a function to the column *names* for all (default) or
selected columns. For example, to make all columns start with a capital
case:


```{r}
library(stringr)
starwars %>% rename_with(str_to_title)
```
...
Sometimes you want to go over two or more vectors or lists at the same time. 
...
map2_chr(sw_names, sw_vehcls, 
  ~ paste(.x, "has", length(.y), "vehicle(s)")) %>%
head
```

## This last example...

Can we improve it? "has no vehicles", "has 1 vehicle", "has 2 vehicles"?
...
```{r}
nums <- 1:10
ifelse(nums %% 2 == 0, "even", "uneven")
```

`ifelse` is a vectorized version of if. It takes three arguments of either
the same length or length 1.

## ifelse with expression data


```{r}
res <- read_csv("../Datasets/transcriptomics_results.csv") %>%
  select(GeneName, ends_with("F.D1"))
res <- res %>%
  mutate(DEG=ifelse(qval.F.D1 < 0.05 & abs(logFC.F.D1) > 2, TRUE, FALSE))
```
...
sw_names <- starwars$name
sw_vehcls <- starwars$vehicles
vehcl_n <- map(sw_vehcls, length)

vehcl_n <- ifelse(vehcl_n == 0, "no vehicles",
                  ifelse(vehcl_n > 1, paste(vehcl_n, "vehicles"),
                                      paste(vehcl_n, "vehicle")))

paste(sw_names, "has", vehcl_n) %>% head
```
...
## `imap_*` – useful for rbinding data frames


```{r}
dfs <- list(
            condA=data.frame(ID=1:3, res=rnorm(3)),
            condB=data.frame(ID=1:3, res=rnorm(3)))
